...
function agregarItemAlFinalDelArray(array, elemento) {array[array.length] = elemento;
    return array
...
function arrayContiene(array, elemento) {for(var i = 0; i < array.length; i++) {
  if(array[i] === elemento) {
    return true;
  }
}
...
function numeroMasGrande(numeros) {   var Mayor = numeros[0];
...
function multiplicarArgumentos() {if(arguments.length < 1 ) return 0; 
   var total = 1 
     for(var i=0;i<arguments.length;i++){total=total * arguments[i];}return total
...
function cuentoElementos(arreglo){let contador = 0 
    for(let i=0;i< arreglo.length;i++){if(arreglo[i]>19){contador ++}
     }return contador 
...
function diaDeLaSemana(numeroDeDia) {  if(numeroDeDia === 1 || numeroDeDia === 7 ){
...
function todosIguales(arreglo) {for(var i=0; i<arreglo.length -1; i++)if (arreglo[i] !== arreglo [i+1])
...
function mayorACien(array) { var nuevoArray= []
  for(let i=0;i< array.length; i++){ if (array[i] > 100 ) {nuevoArray.push(array[i])}
}return nuevoArray
